<template>
    ...
    <div class="relative w-full max-w-6xl mx-auto">
      ...
      <div class="overflow-hidden relative h-96">
        ...
        <div class="md:hidden h-full">
          <div v-for="(certificate, index) in listOfCertificates" :key="index"
            class="absolute inset-0 transition-transform duration-500 ease-in-out transform" :class="{
      ...
      <div class="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-20">
        <div v-for="(_, index) in listOfCertificates" :key="index"
          class="w-3 h-3 rounded-full cursor-pointer transition-colors"
          :class="index === currentIndex ? 'bg-accent-content' : 'bg-info'" @click="goToSlide(index)" />
      </div>
    </div>
  ...
  <div v-if="selectedCertificate"
    ...
    <div class="rounded-box max-w-4xl w-full">
      ...
      <img loading="lazy" :src="selectedCertificate.image_path" :alt="selectedCertificate.title"
